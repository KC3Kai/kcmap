...
function extract() {
  const xmlData = _fsExtra2.default.readFileSync("swf.xml");
  const xml = _libxmljs2.default.parseXml(xmlData);
    ...
    for (const itemRef of itemRefs) {
      try {
        ...
        if (m != null) {
          ...
          const shapeRef = sprite ? sprite.get('subTags/item[@characterId]') : null; // shape
...
function fitting() {
  // Fit spots
  const nsmap = {};
  _lodash2.default.forIn(SPOTS, ({ name }, id) => {
    if (nsmap[name] == null) nsmap[name] = [];
    nsmap[name].push(id);
  });
  _lodash2.default.forIn(nsmap, (ids, name) => {
    ...
    for (const oid of ids) {
      if (oid === id) continue;
      delete SPOTS[oid];
      _lodash2.default.forIn(ROUTE, (route, id) => {
        const { start, end } = route;
        if (start && start.join() === oid) route.start = idCoord;
        if (end && end.join() === oid) route.end = idCoord;
      });
    }
  });
  // Fit Route
  _lodash2.default.forIn(ROUTE, ({ start }, id) => {
    if (start == null) return;
    const distance = {};
    let mid = null,
        mdst = null; // id of minimum distance
    _lodash2.default.forIn(SPOTS, ({ coord }, id) => {
    ...
    _lodash2.default.forIn(distance, (dst, did) => {
      if (did === mid) return;
...
function addSpotName() {
  if (!_fsExtra2.default.existsSync('spots.json')) {
    _fsExtra2.default.writeJSONSync('spots.json', {});
  }
  const named = JSON.parse(_fsExtra2.default.readFileSync('spots.json'));
  const unamed = {};
  _lodash2.default.forIn(SPOTS, (spot, id) => {
    if (named[id] != null) {
      ...
      unamed[id] = [0.05*spot.coord[0], 0.05*spot.coord[1]].join('_');
      SPOTS[id].name = unamed[id];
    }
  });
  if (Object.keys(unamed).length > 0) {
    _fsExtra2.default.writeJSONSync('spots_unamed.json', unamed);
    console.warn([`Unamed spot found! Please set their name in "spots.json"`].join('\n'));
  }
}

function addSpotDistance() {
  if (!_fsExtra2.default.existsSync('celldata.json')) {
    throw new Error(`celldata.json not found!`);
  }
  const MSAPI = _fsExtra2.default.readJSONSync('celldata.json');
  for (const _ref of MSAPI.api_cell_data) {
    const { api_no, api_distance } = _ref;

    if (api_no == null || api_distance == null) continue;
    const route = ROUTE[api_no];
    const id = route.end.join();
    const spot = SPOTS[id];
    if (spot.tag.includes(api_distance) === false) spot.tag.push(api_distance);
  }
}
...
<defs>
  ...
  _lodash2.default.forIn(ROUTE, ({ start, end }, id) => {
    if (start == null) return;
...
function drawSpots() {
  _lodash2.default.forIn(SPOTS, ({ coord, start, name, tag }) => {
...
function drawSpotIcons() {
  ...
  const MSAPI = _fsExtra2.default.readJSONSync('celldata.json');
  for (const _ref2 of MSAPI.api_cell_data) {
    const { api_no, api_color_no } = _ref2;

    if (api_color_no < 2 || api_color_no > 10) continue;
    const route = ROUTE[api_no];
...
function drawDone() {
  if (DRAWS.length > 0) {
    _fsExtra2.default.writeFileSync('draw.svg', `<?xml version="1.0"?>
...
function genpoi() {
  const route = {};
  _lodash2.default.forIn(ROUTE, ({ start, end }, id) => {
    route[id] = [start ? SPOTS[start.join()].name : null, end ? SPOTS[end.join()].name : null];
  });
  const spots = {};
  _lodash2.default.forIn(SPOTS, ({ coord, start, name }, id) => {
    let type = start ? 'start' : '';
    spots[name] = [coord[0], coord[1], type];
  });
  _fsExtra2.default.writeJSONSync('poi.json', { route, spots });
}
